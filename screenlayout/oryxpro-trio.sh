...
if [[ $CONNECTED = *"DP-3.8"* ]]; then
	...
	xrandr --output DP-3.8 --auto --panning 3840x2160+3840+0 --right-of DP-0
	DP="DP-3.8"
elif [[ $CONNECTED = *"DP-3"* ]]; then
	echo "Setting Asus on DP-3 to right of laptop display"
	xrandr --output DP-3 --auto --panning 3840x2160+3840+0 --right-of DP-0
	DP="DP-3"
fi
...
if [[ $CONNECTED = *"HDMI-0"* ]]; then
	...
	xrandr --output HDMI-0 --auto --panning 3840x2160+7680+0 --right-of $DP;
fi
